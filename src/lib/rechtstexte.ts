...
function versandZahlung(sprache: Sprache): string {
  if (sprache === 'fr') {
    return [
      ...
      'Paiement',
      "Le paiement se fait via PayPal — avec un compte PayPal, une carte ou un prélèvement ; un compte PayPal n'est pas nécessaire. Le montant total est dû en une seule fois à la commande ; la fabrication commence une fois le paiement reçu.",
      'Prix',
  ...
  if (sprache === 'en') {
    return [
      ...
      'Payment',
      'Payment is handled by PayPal — with a PayPal account, a card or direct debit; a PayPal account is not required. The full amount is due in one go when you order; production starts once payment has arrived.',
      'Prices',
  ...
  return [
    ...
    'Zahlung',
    'Bezahlt wird über PayPal — mit PayPal-Konto, Karte oder Lastschrift; ein PayPal-Konto ist dafür nicht nötig. Der volle Betrag ist mit der Bestellung in einem Schritt fällig; gefertigt wird nach Zahlungseingang.',
    'Preise',
