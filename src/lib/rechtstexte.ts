...
function versandZahlung(sprache: Sprache): string {
  if (sprache === 'fr') {
    return [
      ...
      'Paiement',
      'Carte bancaire, Apple Pay, Google Pay et Klarna via Stripe, ou PayPal. Le paiement est dû à la commande ; la fabrication commence une fois le paiement reçu.',
      'Prix',
  ...
  if (sprache === 'en') {
    return [
      ...
      'Payment',
      'Card, Apple Pay, Google Pay and Klarna via Stripe, or PayPal. Payment is due on ordering; production starts once payment has arrived.',
      'Prices',
  ...
  return [
    ...
    'Zahlung',
    'Karte, Apple Pay, Google Pay und Klarna über Stripe oder PayPal. Die Zahlung ist mit der Bestellung fällig; gefertigt wird nach Zahlungseingang.',
    'Preise',
