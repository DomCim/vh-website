  ...
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        `  ${' '.repeat(medienOrdner.length + 3)}${' '.repeat(Math.max(1, 16 - medienOrdner.length))}darin ${medienOrdner}/belege mit den festgeschriebenen Rechnungen,`,
        `  ${' '.repeat(medienOrdner.length + 3)}${' '.repeat(Math.max(1, 16 - medienOrdner.length))}Angeboten und Mahnungen als PDF (Aufbewahrungsfrist!)`,
        '',
        'Zurückspielen:',
        ...
        '',
        `Wichtig: In ${medienOrdner}/belege liegen die Belege so, wie sie hinausgegangen sind.`,
        'Sie unterliegen der Aufbewahrungsfrist und dürfen nicht aufgeräumt werden — aus der',
        'Datenbank allein lassen sie sich nicht wiederherstellen.',
      ].join('\n'),
      'utf8',
    )
